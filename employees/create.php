...
requireLogin();
requireRole([1, 2]); // Jen Admin (1) a Manager (2) mohou vytvářet
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        if ($stmt->fetchColumn() > 0) {
            $errors[] = 'Zaměstnanec s tímto emailem již existuje';
        }
    }
    ...
    if (empty($errors)) {
        try {
            ...
            logAction($pdo, 'CREATE', 'employee', $employeeId, [], [
            ...
            setFlash('success', 'Zaměstnanec byl úspěšně přidán');
            redirect('list.php');
...
<head>
    ...
    <title>Přidat zaměstnance - Správa Zaměstnanců</title>
    ...
    <div class="wrapper">
        ...
        <nav class="sidebar">
            ...
            <ul class="menu">
                ...
                <li><a href="create.php" class="active"><i class="fas fa-plus"></i> Přidat zaměstnance</a></li>
        ...
        <main class="content">
            ...
            <header class="page-header-actions">
                <div>
                    <h1><i class="fas fa-plus"></i> Přidat nového zaměstnance</h1>
                    <p>Vyplňte všechny povinné údaje</p>
                </div>
            </header>
                            ...
                            <div class="form-group">
                                ...
                                <select id="status" name="status" required>
                                    <?php foreach (getStatuses() as $key => $value): ?>
                                        <option value="<?= $key ?>" 
                                            <?= ($_POST['status'] ?? 'active') === $key ? 'selected' : '' ?>>
                                            <?= escape($value) ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <input type="number" id="salary" name="salary" min="0" step="1000" required
                                       value="<?= escape($_POST['salary'] ?? '') ?>">
                            </div>
                            
                            <div class="form-group">
                                <label for="hire_date">Datum nástupu *</label>
                                <input type="date" id="hire_date" name="hire_date" required
                                       value="<?= escape($_POST['hire_date'] ?? date('Y-m-d')) ?>">
                            </div>
                        </div>
                    </section>
                    
                    <div class="form-actions">
                        <button type="submit" class="btn btn-primary"><i class="fas fa-floppy-disk"></i> Uložit zaměstnance</button>
